...
class GaussianDiffPlaneWave(SourcePlaneWave):
    ...
    def __init__(self, magnitude, Md, Np, dt, eps=1.0, mu=1.0, Sc=1.0):
        '''
        magnitude - максимальное значение в источнике;
        dg - коэффициент, задающий начальную задержку Вейвлет Рикера;
        wg - коэффициент, задающий ширину Вейвлет Рикера.
        '''
    ...
    def getE(self, m, q):
        return (1-2 * np.pi**2 * ((Sc * q + m) / (Np) - Md)**2)*np.exp(-np.pi**2 * ((Sc * q + m) / (Np) - Md)**2)
...
if __name__ == '__main__':
    ...
    # Параметры слоев
    layers_cont = [LayerContinuous(0, eps=3.3, sigma=0.0)]
